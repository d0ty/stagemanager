...
import { Plus, Trash2, CheckCircle2, Circle, Mic2, Edit2, User, ChevronDown, ChevronUp } from 'lucide-react';
...
export default function SoundTech() {
  ...
  const { data: equipments } = useQuery({
    queryKey: ['equipments'],
    queryFn: () => base44.entities.Equipment.filter({category: 'hangtechnika'}),
    initialData: []
  });
  ...
  // If no program selected (or "all" - though current UI forces selection), maybe show unassigned?
  // But currently UI forces selection. Let's allow selecting "none" in sidebar to show unassigned tasks.

  // Mutations
  const createTaskMutation = useMutation({
    mutationFn: (data) => base44.entities.SoundTask.create(data),
    onSuccess: () => queryClient.invalidateQueries(['sound-tasks'])
  });
  ...
  const updateTaskMutation = useMutation({
    mutationFn: ({ id, data }) => base44.entities.SoundTask.update(id, data),
    onSuccess: () => queryClient.invalidateQueries(['sound-tasks'])
  });

  const deleteTaskMutation = useMutation({
    mutationFn: (id) => base44.entities.SoundTask.delete(id),
    onSuccess: () => queryClient.invalidateQueries(['sound-tasks'])
  });
    ...
    <div className="space-y-8 animate-in fade-in">
      <div>
        <h1 className="text-3xl font-bold text-slate-900 flex items-center gap-2">
          ...
          Hangtechnikai Feladatok
        </h1>
        <p className="text-slate-500">Technikai igények és setup feladatok kezelése</p>
      </div>
                ...
                <Card>
                  ...
                  <CollapsibleContent>
                    ...
                    <div className="grid gap-2">
                      ...
                      <Input name="title" required placeholder="pl. Mikrofonozás, Soundcheck" />
                    </div>
                ...
                {programTasks.map(task => (
                  <div key={task.id} className="group flex items-center justify-between p-4 bg-white rounded-lg border border-slate-200 shadow-sm hover:shadow-md transition-all">
                    <div className="flex items-start gap-4 flex-1">
                      <button onClick={() => updateTaskStatusMutation.mutate({ 
                        id: task.id, 
                        status: task.status === 'kesz' ? 'teendo' : 'kesz' 
                      })}>
                        {task.status === 'kesz' ? (
                      ...
                      <div className="flex-1 cursor-pointer" onClick={() => setViewingTask(task)}>
                        <h4 className={`font-medium hover:text-indigo-600 transition-colors ${task.status === 'kesz' ? 'line-through text-slate-400' : 'text-slate-900'}`}>
                          {task.title}
                        </h4>
                        <div className="flex items-center gap-2 mt-1">
                          <Badge variant="outline" className="text-xs flex items-center gap-1 bg-indigo-50 text-indigo-700 border-indigo-200">
                    ...
                    <div className="flex flex-col gap-1 opacity-0 group-hover:opacity-100">
                        {can('tasks', 'edit') && (
                        <Button variant="ghost" size="icon" className="text-slate-400 hover:text-indigo-600" onClick={() => setEditingTask(task)}>
            ...
            <div className="space-y-4 mt-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="grid gap-2">
                  ...
                  <Badge variant="outline" className="w-fit flex items-center gap-1 bg-indigo-50 text-indigo-700 border-indigo-200">
                    <User className="w-3 h-3" />
              ...
              <div className="flex gap-2 pt-4 border-t">
                ...
                <Button onClick={() => setViewingTask(null)} className="flex-1 bg-indigo-600">
                  Bezárás
                </Button>
              </div>
            </div>
