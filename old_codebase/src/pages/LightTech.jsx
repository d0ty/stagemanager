...
import { Plus, Trash2, CheckCircle2, Circle, Lightbulb, Edit2, User, ChevronDown, ChevronUp } from 'lucide-react';
...
export default function LightTech() {
  ...
  const { data: equipments } = useQuery({
    queryKey: ['equipments'],
    queryFn: () => base44.entities.Equipment.filter({category: 'fenytechnika'}),
    initialData: []
  });
  ...
  const createTaskMutation = useMutation({
    mutationFn: (data) => base44.entities.LightTask.create(data),
    onSuccess: () => queryClient.invalidateQueries(['light-tasks'])
  });

  const updateTaskMutation = useMutation({
    mutationFn: ({ id, data }) => base44.entities.LightTask.update(id, data),
    onSuccess: () => queryClient.invalidateQueries(['light-tasks'])
  });

  const deleteTaskMutation = useMutation({
    mutationFn: (id) => base44.entities.LightTask.delete(id),
    onSuccess: () => queryClient.invalidateQueries(['light-tasks'])
  });
  ...
  return (
    <div className="space-y-8 animate-in fade-in">
      <div>
        ...
        <p className="text-slate-500">Világítási tervek és setup feladatok</p>
      </div>
        ...
        <div className="lg:col-span-3 space-y-6">
          {activeProgram || selectedProgramId === 'unassigned' || selectedProgramId === 'all' ? (
                ...
                <Card>
                  ...
                  <CollapsibleContent>
                    ...
                    <div className="grid gap-2">
                      ...
                      <Input name="title" required placeholder="pl. Derítés beállítása" />
                    </div>
                    ...
                    {pendingEquipmentId && (() => {
                      ...
                      return (
                        ...
                        <Button type="submit" className="bg-amber-600 hover:bg-amber-700 text-white w-full">
                          <Plus className="w-4 h-4 mr-2" /> Feladat Hozzáadása
                        </Button>
                      </form>
                    </CardContent>
                  </CollapsibleContent>
              ...
              <div className="grid gap-3">
                {programTasks.length === 0 && (
                  <div className="text-center py-10 text-slate-400">
                    {selectedProgramId === 'all' ? 'Nincsenek feladatok.' : selectedProgramId === 'unassigned' ? 'Nincsenek általános feladatok.' : 'Ehhez az eseményhez nincsenek feladatok.'}
                  </div>
                )}
                {programTasks.map(task => (
                  <div key={task.id} className="group flex flex-col sm:flex-row sm:items-center justify-between p-4 bg-white rounded-lg border border-slate-200 shadow-sm hover:shadow-md transition-all gap-4">
                    <div className="flex items-start gap-4 flex-1">
                      ...
                      <div className="flex-1 cursor-pointer" onClick={() => setViewingTask(task)}>
                        <h4 className={`font-medium hover:text-amber-600 transition-colors ${task.status === 'kesz' ? 'line-through text-slate-400' : 'text-slate-900'}`}>
                          {task.title}
                        </h4>
                        <div className="flex flex-wrap items-center gap-2 mt-2">
                          ...
                          <Badge className={`text-[10px] ${
                            task.priority === 'kritikus' ? 'bg-red-100 text-red-800' :
                            task.priority === 'magas' ? 'bg-orange-100 text-orange-800' :
                    ...
                    <div className="flex flex-col gap-1">
                        {can('tasks', 'edit') && (
                        <Button variant="ghost" size="icon" className="text-slate-400 hover:text-amber-600" onClick={() => setEditingTask(task)}>
            ...
            <div className="flex items-center justify-center h-64 bg-slate-50 rounded-xl border-2 border-dashed">
              <p className="text-slate-400">Válassz egy eseményt.</p>
            </div>
          )}
        </div>
      </div>
            ...
            <div className="space-y-4 mt-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="grid gap-2">
                  ...
                  <Badge variant="outline" className="w-fit flex items-center gap-1 bg-amber-50 text-amber-700 border-amber-200">
                    <User className="w-3 h-3" />
              ...
              <div className="flex gap-2 pt-4 border-t">
                ...
                <Button onClick={() => setViewingTask(null)} className="flex-1 bg-amber-600">
                  Bezárás
                </Button>
              </div>
            </div>
      ...
      <Dialog open={!!editingTask} onOpenChange={(open) => {
        ...
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleUpdateTask} className="space-y-4 mt-4">
                ...
                <Button type="submit" className="w-full bg-amber-600 text-white">Mentés</Button>
            </form>
        </DialogContent>
      </Dialog>
    </div>
